...
import { LakeBoards } from './boards.js';
import { LakeTiming } from './timing.js';
import { LakeService } from './service.js';
import { LakeHarvest } from './harvest.js';
import { LakeGuardrail } from './guardrail.js';
import { LakeRoadworks } from './roadworks.js';
...
class LakeLevel {
  ...
  async _build() {
    ...
    this.boards = new LakeBoards(this.terrain, tier); scene.add(this.boards.root);
    this.timing = new LakeTiming(this.terrain, tier); scene.add(this.timing.root);
    this.service = new LakeService(this.terrain, tier); scene.add(this.service.root);
    this.harvest = new LakeHarvest(this.terrain, tier); scene.add(this.harvest.root);
    this.guardrail = new LakeGuardrail(this.terrain, tier, this.ctx.collision); scene.add(this.guardrail.root);
    this.roadworks = new LakeRoadworks(this.terrain, tier, this.ctx.collision); scene.add(this.roadworks.root);
  ...
  materials() { return [this.terrainMat, ...this.road.materials, ...this.water.standardMaterials, ...this.veg.materials, ...this.props.materials, ...this.shelter.materials, ...this.fences.materials, ...this.farm.materials, ...this.roadside.materials, ...this.structures.materials, ...this.rock.materials, ...this.wayside.materials, ...this.stage.materials, ...this.boards.materials, ...this.timing.materials, ...this.service.materials, ...this.harvest.materials, ...this.guardrail.materials, ...this.roadworks.materials, ...this.fauna.materials, ...this.distance.materials]; }
  ...
  stats() { return { water:this.water?.stats(), flora:this.veg?.stats(), props:this.props?.stats(), shelter:this.shelter?.stats(), fences:this.fences?.stats(), farm:this.farm?.stats(), roadside:this.roadside?.stats(), structures:this.structures?.stats(), rock:this.rock?.stats(), wayside:this.wayside?.stats(), stage:this.stage?.stats(), boards:this.boards?.stats(), timing:this.timing?.stats(), service:this.service?.stats(), harvest:this.harvest?.stats(), guardrail:this.guardrail?.stats(), roadworks:this.roadworks?.stats(), dust:this.dust?.stats(), colliders:this.colliderStats, fauna:this.fauna?.stats(), distance:this.distance?.stats() }; }
